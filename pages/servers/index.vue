...
const listScenarios = computed(() => {
    return data.value.servers?.map(server => { 
...
<template>
    <div class="container mx-auto my-6">
        <div class="flex flex-col md:flex-row gap-4 justify-between">
            <h1>Serveurs</h1>
            <div class="flex flex-col sm:flex-row gap-2 items-start sm:items-center" v-if="data?.servers">
                <select v-model="scenarioFilter" class="select select-bordered select-sm">
                    <option value="">-- Scénario --</option>
                    <option v-for="scenario in listScenarios" :key="scenario.id" :value="scenario.id">{{ scenario.name }}</option>
                </select>
                <select v-model="difficultyFilter" class="select select-bordered select-sm">
                    <option value="">-- Difficulté --</option>
                    <option v-for="difficulty, key in difficulties" :key="key" :value="key">{{ difficulty }}</option>
                </select>
                <label class="flex gap-2 items-center">
                    <input type="checkbox" class="toggle toggle-sm toggle-accent" v-model="onlyOpen">
                    Ouverts uniquement
                </label>
            </div>
        </div>
